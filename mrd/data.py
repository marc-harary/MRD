...
class MNISTData(pl.LightningDataModule):
    def __init__(
        self, data_root: str, train_bs: int, test_bs: int, num_workers: int = 4
    ):
        super().__init__()
    ...
    def setup(self, stage: Optional[str] = None):
        full = datasets.MNIST(
            self.data_root, train=True, download=False, transform=self.tfm
        )
        test = datasets.MNIST(
            self.data_root, train=False, download=False, transform=self.tfm
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            drop_last=True,
            persistent_workers=(ACCELERATOR == "gpu"),
        )
...
def target_rbf_bumps(
    x: torch.Tensor, centers: torch.Tensor, scales: torch.Tensor, weights: torch.Tensor
) -> torch.Tensor:
...
def target_polynomial(
    x: torch.Tensor, W2: torch.Tensor, b: torch.Tensor
) -> torch.Tensor:
...
class AnalyticRegressionDataset(Dataset):
    ...
    def __init__(
        ...
        if x_dist == "normal":
            ...
            X = (2 * uniform_a) * torch.rand(
                n, d
            ) - uniform_a  # , generator=g, dtype=dtype) - uniform_a
        else:
...
class InfiniteAnalyticRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            if self.noise_std > 0:
                y = y + self.noise_std * torch.randn(
                    ()
                )  # , generator=g, dtype=self.dtype)
